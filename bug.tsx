...
import { Search, Download, Copy, Trash2, Save, Filter, X, Check, Edit } from 'lucide-react';
...
export default function ExcelLikeTable({
  ...
  const systemColumns = ['id', 'comments', 'createdAt', 'updatedAt'];
  ...
  // Separate left system columns from other columns
  const leftColumns = LeftSystemColumns.filter(col => !columns.includes(col));
  const dataColumns = columns;
  const rightSystemColumns = systemColumns.filter(col => !columns.includes(col));
  
  // Combine all columns for processing
  const allColumns = [...leftColumns, ...dataColumns, ...rightSystemColumns];
  ...
  const executeBatchUpdate = () => {
    if (selectedCells.size === 0 || !batchValue) return;
    ...
    selectedCells.forEach(cellId => {
      ...
      updates.push({ id: record.id, updates: recordUpdate });
      processedRecords.add(record.id);
    });
  ...
  const renderHeaderCell = (column: string, colIndex: number, isFixed: boolean = false) => (
    <th 
      key={column} 
      className={`text-left py-2 px-3 font-medium text-gray-700 border-r min-w-[120px] ${
        isFixed ? 'sticky bg-gray-50 z-20' : ''
      }`}
      style={isFixed ? { left: `${colIndex * 120}px` } : {}}
    >
  ...
  const renderCell = (record: AccessRecord, column: string, rowIndex: number, colIndex: number, isFixed: boolean = false) => {
        ...
        {filteredRecords.length === 0 ? (
          ...
          <div className="overflow-auto max-h-[600px] border rounded relative">
            <table ref={tableRef} className="w-full text-sm">
              <thead className="sticky top-0 bg-gray-50 border-b z-30">
                <tr>
                  {/* Fixed Left System Columns */}
                  {leftColumns.map((column, colIndex) => 
                    renderHeaderCell(column, colIndex, true)
                  )}
                  {/* Scrollable Data Columns */}
                  {dataColumns.map((column, colIndex) => 
                    renderHeaderCell(column, leftColumns.length + colIndex, false)
                  )}
                  {/* Right System Columns */}
                  {rightSystemColumns.map((column, colIndex) => 
                    renderHeaderCell(column, leftColumns.length + dataColumns.length + colIndex, false)
                  )}
                </tr>
              </thead>
              <tbody>
                {filteredRecords.map((record, rowIndex) => (
                  <tr key={record.id} className="border-b hover:bg-gray-50">
                    {/* Fixed Left System Columns */}
                    {leftColumns.map((column, colIndex) => 
                      renderCell(record, column, rowIndex, colIndex, true)
                    )}
                    {/* Scrollable Data Columns */}
                    {dataColumns.map((column, colIndex) => 
                      renderCell(record, column, rowIndex, leftColumns.length + colIndex, false)
                    )}
                    {/* Right System Columns */}
                    {rightSystemColumns.map((column, colIndex) => 
                      renderCell(record, column, rowIndex, leftColumns.length + dataColumns.length + colIndex, false)
                    )}
                  </tr>
                ))}
              </tbody>
        ...
        {canEdit && (
          <div className="mt-4 text-sm text-gray-600">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Select multiple cells and use "Batch Edit" for bulk operations</li>
              <li><strong>Left system columns are fixed and will remain visible when scrolling horizontally</strong></li>
            </ul>
          </div>
        )}
      </CardContent>
      ...
      <Dialog open={showBatchDialog} onOpenChange={setShowBatchDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update {selectedCells.size} selected cells with the same value.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={executeBatchUpdate} disabled={!batchValue}>
              ...
              Update {selectedCells.size} Records
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
