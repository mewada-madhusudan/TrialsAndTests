...
from PyQt6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QScrollArea, QStackedLayout, QSpacerItem, QMenu, QStackedWidget, QDialog)
...
class MainWindow(QMainWindow): # 2 usages ± Mewada, Madhusudan
    ...
    def __init__(self, df, cost_center, userdata): # ± Mewada, Madhusudan
        """
        default class method to intializeall required ui elements for MainWindow widget of PSLV
        :param df:
        """
        super().__init__()
        self.all_tiles = []
        ...
        for label, icon_name in self.user_details:
            ...
            sidebar_layout.addWidget(detail_widget)
        ...
        """)
        # Only show the button for managers
        ...
        if len(self.access) != 0 and self.is_gfbm_user:
            self.access_control_button.setChecked(False)
    ...
    def is_administrator(self):  # 1 usage = Mewada, Madhusudan
        """
        Method checks whether user is admin or not
        :return: bool
        """
        try:
            ...
            if len(managed_lob) > 0:
                return True
        except:
            self.access_control_widget = QDialog()
            return False

    def show_access_control(self):  # 1 usage = Mewada, Madhusudan
        """
    ...
    def get_user_details(self):  # 1 usage = Mewada, Madhusudan
        """
        this method calls phonebook api and gets user details to populate
        :return:
        """
        try:
    ...
    def refresh_applications(self):  # 1 usage = Mewada, Madhusudan
        ...
        try:
            ...
            try:
                ...
                print(e)
                processed_df.to_excel(excel_writer=f"{os.environ.get('USERPROFILE')}/{BACKUP_PATH}/{BACKUP_FILE_NAME}",
                                      index=False)
        except Exception as e:
    ...
    def is_gfbm_user_check(self):  # 1 usage = Mewada, Madhusudan
        """
        function to read the list of cost center from the repository
        :return:
        """
