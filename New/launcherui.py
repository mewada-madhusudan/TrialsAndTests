...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.uat_scroll = QScrollArea()

        for scroll in [self.prod_scroll, self.uat_scroll]:
    ...
    def load_applications(self):
        ...
        # Debug print to check database results
        print("Loading applications for user:", self.username)

        try:
            # Load PROD applications
            prod_apps = self.db.get_user_applications(self.username, "PROD")
            uat_apps = self.db.get_user_applications(self.username, "UAT")
            prod_apps = prod_apps + uat_apps
            print(f"Found {len(prod_apps)} PROD applications")
            for i, app_data in enumerate(prod_apps):
                name, desc, shared_path, env, release_date, validity_period, version_number, registration_id = app_data
                print(f"Creating PROD tile for: {name}")
                ...
                self.prod_layout.addWidget(tile, i // 3, i % 3)
    ...
    def filter_applications(self, text):
        # Filter PROD applications
        self.filter_layout(self.prod_layout, text)
        # Filter UAT applications
        self.filter_layout(self.uat_layout, text)

    def filter_layout(self, layout, text):
        for i in range(layout.count()):
            item = layout.itemAt(i)
            if item and isinstance(item.widget(), ApplicationTile):
                widget = item.widget()
                if text.lower() in widget.app_name.lower():
